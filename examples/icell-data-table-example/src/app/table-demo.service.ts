...
import { DataWithQueryResponseDetails, QueryRequestDetails } from '@i-cell/data-table';
...
export class TableDemoService {
  ...
  getData(queryParams: QueryRequestDetails, sortColumn: any, sortOrder: any) {
    const page = queryParams.page - 1;
    return new Observable<DataWithQueryResponseDetails>((observer) => {
      setTimeout(() => {
        observer.next({
          list: orderBy([...this.data], [sortColumn], [sortOrder]).slice(
            page * queryParams.rows,
            page * queryParams.rows + queryParams.rows
          ),
          paginationParams: {
            totalRows: this.data.length,
            maxPage: this.data.length % queryParams.rows,
          },
